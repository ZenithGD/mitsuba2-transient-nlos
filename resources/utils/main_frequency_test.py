...
if __name__ == "__main__":
    ...
    streakimg = streakimg[:, 40, :, 0]  # only red

    ini = time.time()
    
    transformed = np.fft.fftshift(np.fft.fft(streakimg, axis=1), axes=1)

    elapsed = time.time() - ini
    print(f"took {elapsed} secs.")

    
    plt.imshow(np.real(transformed), cmap='hot')
    plt.savefig("real.png")

    plt.imshow(np.imag(transformed), cmap='hot')
